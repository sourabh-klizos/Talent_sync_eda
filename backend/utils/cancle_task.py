...
async def delete_queued_task(stream_name: str, target_batch_id: uuid.UUID) -> bool:
    ...
    # if not is_completed:
    is_deleted = await delete_task_from_queue(
        stream_id=stream_id, stream_name=stream_name, target_batch_id=target_batch_id
    )
    if is_deleted:
        await cleanup_cancelled_task_dirs(extraxted_dir_list)
...
async def check_task_already_completed(batch_id: str) -> bool:
    ...
    upload_count = await batches.find_one(
        {"batch_id": batch_id}, {"upload_count": 1, "_id": 0}
    )
    ...
    if upload_count:
        ...
        if length == upload_count.get("upload_count"):
            logger.info(
                f"All {upload_count} files processed for batch_id: {batch_id}. Task is complete."
            )
            return True
        else:
            logger.info(
                f"Only {length}/{upload_count} files processed for batch_id: {batch_id}. Task not complete."
            )
